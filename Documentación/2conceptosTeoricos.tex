...
\figuraCaptionCorta{1}{imgs/saliency.png}{Ejemplo de Saliency Map. La imagen original a la izquierda,
con el correspondiente saliency map a la derecha}{saliency}{}{Ejemplo de Saliency Map}

\subsection{Segmentación}\label{segmentacion}
Además de calcular ciertas características de las imágenes, como hemos visto en el apartado anterior, se hace necesaria realizar una segmentación de la imagen, para aumentar la precisión de la detección de los defectos, y también para poder extraer características morfológicas y geométricas del defecto que podría, en futuras versiones del proyecto, ayudar a clasificar el tipo y gravedad del defecto.
...
Debido a su poder de discriminación y su simplicidad de cálculo, se ha convertido en un método popular que se usa en varios tipos de aplicaciones, como por ejemplo la clasificación de expresiones faciales \cite{de2011transformaciones}.
...
Las características geométricas que se han empleado son aquellas que se encuentran disponibles en ImageJ \cite{analyzeij}, que son las siguientes:
...
\item \textbf{Circularidad:} descriptor de forma dado por la siguiente fórmula: 
\[\frac{4\pi \times \acute{a}rea}{per\acute{i}metro^2}\]
Un valor de 1 indica que se trata de un círculo perfecto. Según se acerca a cero, indica que la forma es cada vez más alargada.

\item \textbf{Redondez:} descriptor de forma dado por la siguiente fórmula:
\[\frac{4\pi\times \acute{a}rea}{\pi \times semieje\_mayor^2}\]
Es el inverso del cociente entre el semieje mayor y el menor de la mejor elipse que puede ser dibujada en la región.
...
Nos hemos basado en el estudio con varios clasificadores que hicieron los alumnos que desarrollaron la versión de la aplicación que este proyecto esta mejorando y ampliando, así que hemos usado el que ellos consideraron mejor: \textit{Bagging}.
...
\textit{Bagging} (de \textit{Bootstrap Aggregating}) \cite{breiman1996bagging} es un algoritmo que busca combinar predicciones de un cierto tipo de modelo (en nuestro caso, \textit{REPTree}) mediante votación o media, teniendo los modelos el mismo peso.
...
La combinación de estas predicciones se puede hacer con una media de las salidas (regresión) o con una media de las probabilidades de cada clase (clasificación).
...
no. En caso de que se use regresión lineal serán valores entre 0 y 1 (0 Cuando no haya ningún píxel defectuoso en la ventana y 1 cuando lo sean todos, siendo para todos los casos número de píxeles defectuosos/número de píxeles de la ventana). El defecto se identificará gracias a las máscaras mencionadas anteriormente.
...
\figuraConPosicionCaptionCorta{0.7}{imgs/dibujado_defectos.png}{Proceso de dibujado de defectos en nuestro proyecto. \newline
a)Imagen con defectos marcados \newline
b)Imagen binarizada con el área de defectos \newline
c)Imagen con filtro de detección de bordes\newline
d)Resultado final}{procesoBorde}{}{h}{Proceso de dibujado de defectos en nuestro proyecto}
...
En este caso, lo primero que se hace es generar la imagen de umbrales locales. A partir de esta imagen, generamos una lista con las coordenadas de los píxeles que se han marcado como blancos \ver{segunda_opcion}, parte b.

Después, hacemos una inversión de la imagen de umbrales locales para aplicar un análisis de partículas sobre ella, con el objetivo de obtener todas las regiones que destacan, a las que consideramos regiones candidatas de albergar un defecto \ver{segunda_opcion}, parte c. Este análisis de partículas se hace con las clases de \emph{ImageJ}, que va a aplicar ciertas medidas a los objetos que vaya encontrando en una imagen, buscando sus bordes \cite{particleij} \ver{segunda_opcion}, parte d.
...
\figuraConPosicionCaptionCorta{0.85}{imgs/segunda_opcion.png}{Proceso de detección en la segunda opción \newline
a)Imagen original \newline
b)Imagen segmentada con los umbrales locales \newline
c)Inversión de los umbrales locales\newline
d)Identificación de las regiones candidatas}{segunda_opcion}{}{h}{Proceso de detección en la segunda opción}
